...
The sidebar lists the component types provided by the **Dashboards** API but only the ones whose modules are loaded. If other types are needed, add the extra modules. The order and the components on the list can be changed by setting the `components` array in the `editMode` option - API.

In order to add other custom components to the list, please follow the [custom component guide](./dashboards/custom-component).

![edit-mode-sidebar.png](edit-mode-sidebar.png)

After dragging a component into the preferred place in the layout, a sidebar with the [component settings](./dashboards/edit-mode#component-settings) will appear.
...
Below you can see the settings categories for the [Highcharts Component](./dashboards/highcharts-component).
...
If you do not intend to use the connector, you can set the data using the component's internal options. In [Highcharts Component](./dashboards/highcharts-component) you can do this by defining data in the `series.data` option. In the [KPI Component](./dashboards/kpi-component), by setting the `value` option.

Check how the component settings sidebar works in practice using the [edit mode live example](./dashboards/edit-mode#edit-mode-live-example).
...
```
See how it works in the edit mode live example.
