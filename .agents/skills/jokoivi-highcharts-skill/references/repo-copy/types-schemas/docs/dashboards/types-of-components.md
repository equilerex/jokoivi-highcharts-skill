...
- `type` - the type of the component. It can be [`HTML`](#html-component), [`KPI`](#kpi-component), [`Highcharts`](#highcharts-component),
  [`Grid`](#grid-component) or [custom defined](./dashboards/custom-component).
...
Defines the HTML code as a nested [tree structure](./chart-concepts/dataviz-glossary#tree-data-structure).
...
This example exhibits both approaches to defining HTML components.

Custom HTML components are explained in the [Custom Component](./dashboards/custom-component) section below.

See the [HTML Component](./dashboards/html-component) page for detailed information.
...
Data may be provided directly in the chart option, but in a **Dashboards** application, one often wants to share data between components. In this case, the [Data Pool](./dashboards/data-handling) mechanism offers a convenient solution.
...
The above code snippet is part of a Highcharts Component example that uses embedded data.
...
The above code snippet is part of a Highcharts Component example that uses data shared via a `DataConnector`.
...
Here is an example that uses column assignment.
...
See the [HighchartsComponent](./dashboards/highcharts-component) page for detailed information.

### Grid Component

The Grid Component uses [Highcharts Grid](./grid/general) to render its content. The `Grid` module _must_ be imported before the **Dashboards** module as shown here:
...
The above code snippet is part of a Grid example that uses embedded data.
...
The above code snippet is part of a Grid example that uses data shared via a `DataConnector`.

When using the data connector, the `Grid` is automatically populated. If other components are using the same data connector, they will be automatically updated when the user edits a data grid cell. Here is the example.

NB! If the data was created by the [mathModifier](./dashboards/data-handling#datamodifier), cell editing is disabled by default.

See the [Grid Component](./dashboards/grid-component) page for more information.
...
The above code snippet is part of a KPI component example.

See the [KPI Component](./dashboards/kpi-component) page for more information.
