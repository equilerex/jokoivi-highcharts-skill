...
| CSVConnector | `'CSV'` |
| JSONConnector | `'JSON'` |
| GoogleSheetsConnector | `'GoogleSheets'` |
| HTMLTableConnector | `'HTMLTable'` |

Each of those types has its specific options. For example, the CSVConnector
...
The `DataCursor` is used under the hood in the synchronization system of the dashboards. See [synchronization article](./dashboards/synchronize-components) for more information.
