...
Additionally, both components must use the same connector from the [dataPool](./dashboards/data-handling), a prerequisite for all predefined synchronization types to work.
...
|---|---|---|---|---|
|[HTML](./dashboards/html-component)|no|no|no|no|
|[Highcharts](./dashboards/highcharts-component)|yes|yes|yes|no|
|[Grid](./dashboards/grid-component)|yes|yes|yes|no|
|[KPI](./dashboards/kpi-component)|no|yes|no|no|
|[Navigator](./dashboards/navigator-component)|no|yes|no|yes|
...
The options above are only available for the [Navigator Component](./dashboards/navigator-component#crossfilter). Each component can have its options for the predefined syncs. You can find their descriptions in the articles about these components (eg. the [Highlight Sync options for the Highcharts Component](./dashboards/highcharts-component#highlight-sync-options)).
...
By default, all components with a given type of synchronization enabled and sharing the same connector are synchronized. If you want to synchronize only specific components, you can do so using the `group` option, which is available for each type of synchronization.
...
Demo:
<iframe style="width: 100%; height: 651px; border: none;" src="" allow="fullscreen"></iframe>
...
For each component, you can add or overwrite a custom definition of the existing synchronization `handler` and/or `emitter` to expand its functionality. You can also define your synchronization from scratch.
...
To add synchronization to a [custom component](./dashboards/custom-component), define a static field `predefinedSyncConfig` which should contain two options:
...
The below example shows how [custom sync](./dashboards/synchronize-components) between a [Highcharts Component](./dashboards/highcharts-component) and a [custom component](./dashboards/custom-component) works:
<iframe style="width: 100%; height: 651px; border: none;" src="" allow="fullscreen"></iframe>
