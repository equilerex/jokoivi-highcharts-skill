...
Alternatively, the tooltip can be styled with CSS by enabling [styled mode](./chart-design-and-style/style-by-css):
...
*   The header part of the tooltip can be altered using the tooltip.headerFormat. In a shared tooltip, the first series' headerFormat is used. In v12+, locale-aware date names for `point.key` follow the browser's casing (often lower-case), so use `{ucfirst point.key}` if you want a capitalized header.
...
For clickable links inside the tooltip, use tooltip.stickOnContact. This keeps the tooltip visible when moving the pointer from the point to the tooltip content. See an example.
...
formatting](./chart-concepts/labels-and-string-formatting)
