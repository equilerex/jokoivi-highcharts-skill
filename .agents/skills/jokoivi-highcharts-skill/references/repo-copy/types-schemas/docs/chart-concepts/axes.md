...
with a [cartesian coordinate system](./chart-concepts/dataviz-glossary#cartesian-coordinate-system).
...
#### Timezone handling
By default, Highcharts uses UTC for all date-time axes. You can control how dates are interpreted and displayed by setting the chart-level time.timezone option. Date strings without a timezone are interpreted in this timezone. If a date string contains a timezone offset, that offset is respected.
...
2. **Use axis label format strings** via xAxis.labels.format, where `{boundary}` is available in the label context for boundary ticks.
