...
function TooltipFormat() {
  return (
    <>
      ...
      <div data-hc-option="pointFormat">{"X: {point.x}, Y: {point.y}"}</div>
...
Not all option components supports child components. See [Appendix A](./react/component-children) for an overview.

## See also

A complete example combining multiple option components:

<iframe src="" title="Complex Highcharts React chart example with multiple option components"></iframe>
