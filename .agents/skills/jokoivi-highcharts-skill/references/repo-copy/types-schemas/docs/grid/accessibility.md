...
Setting `accessibility.enabled: false` disables all Grid accessibility features, including ARIA attributes, screen reader regions, and ARIA live announcements.
...
You can add an `aria-description` attribute to individual `<th>` table headers by configuring `header[].accessibility.description`. This is especially useful when [grouped headers](./grid/columns/header) are used, as descriptions of header groups provide additional clarity for users relying on assistive technologies.
