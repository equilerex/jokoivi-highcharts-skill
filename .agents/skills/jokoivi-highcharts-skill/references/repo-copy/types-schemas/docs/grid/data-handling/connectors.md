...
local [`DataTable`](./grid/data-handling/clientside#datatable). After that initial load, Grid works with the data through the
...
Use [Server-side data handling](./grid/data-handling/serverside) instead when your backend should handle sorting, filtering, and pagination on demand.

For full connector details and connector-specific options, read the Dashboards
documentation on [Data handling / DataConnector](./dashboards/data-handling#dataconnector).

## API reference

- `data`
- `data.local.connector`
