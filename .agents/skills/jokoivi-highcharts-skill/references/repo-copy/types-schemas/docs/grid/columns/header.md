...
`className` and `format` support templating as described in [Templating](./chart-concepts/templating), and `{id}` references the key in the DataTable.

If you need more advanced formatting that is not supported through templating, use the `formatter` callback function instead. As in Highcharts Core, we recommend `format` when possible. [Read more here...](./chart-concepts/labels-and-string-formatting#formatter-callbacks)
