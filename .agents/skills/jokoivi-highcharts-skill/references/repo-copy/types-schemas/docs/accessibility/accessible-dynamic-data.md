...
The Highcharts suite of products has extensive dynamic support. Changes to the layout, design, and data is supported after the chart has been created, using methods like `chart.update` and `series.setData`. This gives us some interesting challenges for accessibility, particularly for non-visual users.
...
Announce new data
-----------------
To enable announcements of new data and drilldown events for your charts, set the `accessibility.announceNewData.enabled` option.
...
By defining the `accessibility.announceNewData.announcementFormatter` it is possible to add logic that can choose which updates to announce. This enables you to e.g only announce new data that is critical, or for a certain data series.
