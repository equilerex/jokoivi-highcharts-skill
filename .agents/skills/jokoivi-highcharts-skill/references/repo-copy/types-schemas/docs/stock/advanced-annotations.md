...
annotations](./advanced-chart-features/annotations-module).
 
...
Look for the typeOptions and labelOptions for a specific annotation.
...
By default, all control points are set to `visible: false`. To make them visible, change the setting to `true` in the `controlPointOptions`. Read more about controlPointOptions and heightControlPoint.
