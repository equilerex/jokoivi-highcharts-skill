...
Highcharts Maps supports different ways of navigating around the map - zooming, panning, zooming to an area etc. The API options related to this can be viewed at mapNavigation.
...
Setting the map view programmatically is done through the Chart.mapView object. It can be zoomed through the `mapView.zoomBy` function or directly by Point.zoomTo methods.
