...
Note: The following content is only relevant to legacy versions of Highcharts. Since Highcharts v9.3, experimental projection is built in, allowing the use of `lat` and `lon` properties to be handled without the use of proj4js, as well as applying GeoJSON-compliant geometry configuration to points, maplines and map points directly.

<iframe style="width: 100%; height: 550px; border: 0;" src="" allow="fullscreen"></iframe>

Highcharts Maps from version 1.1.0 comes with support for latitude/longitude. This feature requires that the proj4js library has been loaded before Highcharts Maps. The latest version of the _proj4js_ library can be loaded from cdnjs.
...
```
You can also use the Chart.fromLatLonToPoint and Chart.fromPointToLatLon functions to convert between map values and latitude/longitude manually.
...
```
This object contains information necessary to transform the coordinates used in the map to latitude/longitude, and vice versa. Adding this information to your map is easily done with a text editor, assuming your map is stored in GeoJSON format.
...
Highcharts Maps will automatically detect which transform object to use when transforming to and from lat/lon on a map. If you want to manually perform a conversion using a specific transform object, use the Chart.transformToLatLon and Chart.transformFromLatLon functions.
