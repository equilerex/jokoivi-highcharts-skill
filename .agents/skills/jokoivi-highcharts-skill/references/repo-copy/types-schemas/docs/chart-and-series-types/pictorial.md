...
The pictorial series requires the modules/pictorial.js file to be loaded.
...
In the case of `normal` stacking, the shape is sometimes not completely filled with data. It is possible to set a background shape by enabling the yAxis.stackShadow option. It is fully customizable, so it can be set as a fill color or even border.
...
When stacking is not enabled, the series duplicate the image. This can be useful for comparing data within one series. The paths.max option determines the height of the image. It is the ratio of `yAxis.max` to the `paths.max`.
...
API Reference
-------------
For an overview of the pictorial series options see the API reference.
