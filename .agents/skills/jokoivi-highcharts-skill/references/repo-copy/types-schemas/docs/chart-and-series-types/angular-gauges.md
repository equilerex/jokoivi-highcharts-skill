...
*   **`size`** — Diameter of the pane as a percentage of the smaller plot
...
*   **`center`** — `[x, y]` position of the pane center. Defaults to
...
*   **`tickPosition`**
    ...
    outside.
*   **`gridLineWidth`** and **`gridLineColor`** — The radial grid lines that
...
<iframe style="width: 100%; height: 400px; border: none;" src="" allow="fullscreen"></iframe>

A gauge can also have multiple pivots, either as separate series or as points
within the same series. Check out our clock gauge for a demonstration.
...
reference, the gauge API
reference and the
solidgauge API
reference.

Worth checking out:
*   Inspiration dashboard with 12 gauge designs
*   Speedometer with dual axes
*   Simple gauge with rounded plot bands
