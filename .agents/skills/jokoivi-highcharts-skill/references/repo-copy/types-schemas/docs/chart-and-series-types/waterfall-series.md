...
Defining the points in a waterfall is pretty straight forward. Each point is accumulated on top of the next, or subtracted from the sum if it is negative. The most intuitive way of defining a series is to give each point a name and an Y value, and set xAxis.type to _"category"_. A live demo can be seen at www.highcharts.com/demo/waterfall.
